...
The Worker limits searches and daily routes to Europe, accepts exactly one allowlisted lodging type per day, caps search radii, requires strong sightseeing ratings, uses fixed Google field masks, rejects requests without the correct browser origin, and never caches Places responses. Identical OSM routes are cached for 24 hours. The Cloudflare rate-limiting rule should cover both `/api/places` and `/api/route`.
